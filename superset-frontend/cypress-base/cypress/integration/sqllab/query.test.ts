...
describe('SqlLab query panel', () => {
  ...
  it.skip('supports entering and running a query', () => {
    ...
    cy.get('.TableSelector .Select:eq(0) input[type=text]').type('{enter}');

    cy.get('#brace-editor textarea').focus();
    cy.get('#brace-editor textarea').clear();
    cy.get('#brace-editor textarea').type(`{selectall}{backspace}SELECT 1`);
  ...
  it.skip('successfully saves a query', () => {
    ...
    cy.get('#brace-editor textarea').blur();
  ...
  it('Create a chart from a query', () => {
    ...
    cy.get('.ace_text-input').clear({ force: true });
    cy.get('.ace_text-input').type(`{selectall}{backspace}${query}`, {
      force: true,
    });
